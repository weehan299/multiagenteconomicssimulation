...
def run(*args, **kwargs):
    ...
    mixed_strategy_agent1 = Mixed_Strategy_Binary_State_QLearning(memory_length= 1, alpha = kwargs.get("alpha",0.2), gamma = kwargs.get("gamma", 0.95),
                        policy=Boltzmann(lambda0=kwargs.get("lambda0",1000), lambda1=kwargs.get("lambda1",0.999)))
                        #policy=TimeDecliningExploration(beta = kwargs.get("beta", 1e-05)))
    
    mixed_strategy_agent2 = Mixed_Strategy_Binary_State_QLearning(memory_length= 1, alpha = kwargs.get("alpha",0.2), gamma = kwargs.get("gamma", 0.95),
                        policy=Boltzmann(lambda0=kwargs.get("lambda0",1000), lambda1=kwargs.get("lambda1",0.999)))
                        #policy=TimeDecliningExploration(beta = kwargs.get("beta", 1e-05)))
    ...
    env = EconomicEnvironment(
        ...
        agents= agents,
        #reward= MixedStrategyPayoff(agents=agents),
